...
contract Registry is IRegistry, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    mapping(bytes32 => Lending) internal lendings; // keccak256(abi.encodePacked(nftAddress, tokenID, _lendingID)) => Lending
    // mapping(bytes32 => Renting) private rentings;
    mapping(bytes32 => Renting) internal rentings; // keccak256(abi.encodePacked(nftAddress, tokenID, _rentingID)) => Renting

    // 新增以下兩個 mapping 方便查詢
    // mapping (address => mapping(uint256 => Lending)) internal ownerLendings;
    // mapping (address => mapping(uint256 => Renting)) internal userRentings;
    mapping (address => OwnerLending[]) internal ownerLendings;
    mapping (address => UserRenting[]) internal userRentings;
    ...
    function handleLend(IRegistry.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            ownerLendings[msg.sender].push(IRegistry.OwnerLending({
                nftStandard: cd.nftStandard[i],
                lenderAddress: payable(msg.sender),
                lendingID: lendingID,
                maxRentDuration: cd.maxRentDuration[i],
    ...
    function handleStopLend(IRegistry.CallData memory cd) private {
        ...
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            if (ownerLendings[msg.sender][i].lendingID == cd.lendingID[i]) {
                delete ownerLendings[msg.sender][i];
            }
        }
        safeTransfer(
            cd,
    ...
    function handleRent(IRegistry.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            // IRegistry.UserRenting[] storage userRenting = userRentings[msg.sender];
            userRentings[msg.sender].push(IRegistry.UserRenting({
                renterAddress: payable(msg.sender),
                rentingID: rentingID,
                rentAmount: uint16(cd.rentAmount[i]),
                rentDuration: cd.rentDuration[i],
    ...
    function handleStopRent(IRegistry.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            if (userRentings[msg.sender][i].rentingID == cd.rentingID[i]) {
                delete userRentings[msg.sender][i];
            }
        }
    }
